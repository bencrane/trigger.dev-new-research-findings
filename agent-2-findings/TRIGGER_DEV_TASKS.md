...
| `middleware` | `(params) => Promise<void>` | No | -- | Task-level middleware. Wraps the `run` function. <!-- UNVERIFIED --> |
...
| `outputSchema` | `ZodSchema` | No | Zod schema for the return value. Validated after `run()` completes. <!-- UNVERIFIED --> |

> **WARNING**: If the payload does not match the schema, the run will fail immediately without retrying (the error is treated as an `AbortTaskRunError`). <!-- UNVERIFIED -->
...
| `upcoming` | `Date[]` | Array of the next few scheduled run times. <!-- UNVERIFIED --> |
...
> Task-level hooks and global hooks both fire. Global hooks fire first, then task-level hooks. <!-- UNVERIFIED -->
...
| `metadata` | `Record<string, any>` | No | -- | Arbitrary metadata attached to the run. <!-- UNVERIFIED --> |
...
| `maxAttempts` | `number` | No | Task default | Override max retry attempts for this run. <!-- UNVERIFIED --> |
| `priority` | `number` | No | -- | Priority within the queue (higher = sooner). <!-- UNVERIFIED --> |
...
> **WARNING**: There is a depth limit on nested `triggerAndWait` calls. Tasks calling `triggerAndWait` on children that also call `triggerAndWait` will work, but deeply nested chains (typically beyond ~5 levels) may hit platform limits. <!-- UNVERIFIED -->
...
<!-- UNVERIFIED: The exact API for hidden tasks is not confirmed in available documentation. It may use a `hidden: true` option on the task definition. -->
...
| Machine | vCPU | Memory | Best For |
|---|---|---|---|
| `micro` | 0.25 | 0.25 GB | Lightweight HTTP calls, queue workers |
| `small-1x` | 0.5 | 0.5 GB | Default. Most tasks. |
...
| `large-1x` | 2 | 8 GB | Large model inference, video processing |
| `large-2x` | 4 | 16 GB | ML training, massive data sets |

<!-- UNVERIFIED: Exact vCPU and memory values are inferred from naming conventions. Check https://trigger.dev/docs/machines for confirmed specs. -->
...
- [Runs & Run Management](./TRIGGER_DEV_RUNS.md) <!-- UNVERIFIED -->
- [Waits & Tokens](./TRIGGER_DEV_WAITS.md) <!-- UNVERIFIED -->
- [Errors & Retrying](./TRIGGER_DEV_ERRORS_RETRYING.md) <!-- UNVERIFIED -->
- [Deployment & Versioning](./TRIGGER_DEV_DEPLOYMENT.md) <!-- UNVERIFIED -->
- [Environment Variables](./TRIGGER_DEV_ENV_VARS.md) <!-- UNVERIFIED -->
- [REST API Reference](../01-api-docs/)
