...
| **Schedule** | A cron-based recurring trigger for a task. Can be `DECLARATIVE` (defined in code) or `IMPERATIVE` (created via API). Supports timezones. See [Schedules](../01-api-docs/05-schedules-api/01-create-schedule.md). |
| **Wait / Waitpoint** | A durable pause mechanism. The process is checkpointed and freed while waiting. Types: `wait.for()` (duration), `wait.until()` (date), `wait.forToken()` (external callback). See [Waitpoints](../01-api-docs/08-waitpoints-api/00-create-waitpoint-token.md). |
...
// Used exclusively in trigger.config.ts -- see Section 6
```
...
| Operation | Max Items |
|---|---|
| `batchTrigger()` | **1,000** items per call |
| `batchTriggerAndWait()` | **1,000** items per call |

### 8.2 Concurrency Limits (Trigger.dev Cloud)

<!-- UNVERIFIED: Exact limits may vary by plan -->

| Plan | Max Concurrent Runs | Active Preview Branches |
|---|---|---|
| Free | 5 | 0 |
| Hobby | 25 | 5 |
| Pro | 100+ (configurable) | 20 (then paid) |

### 8.3 Payload & Output Sizes

<!-- UNVERIFIED: Check current docs for exact limits -->

| Limit | Value |
|---|---|
| Trigger payload | 10 MB (JSON serialized) |
| Run output | 10 MB (JSON serialized) |
| Metadata | 64 KB |
| Tags per run | 5 |
| Tag length | 128 characters |
| Log entry size | 4 KB per entry |

### 8.4 Schedule Limits

<!-- UNVERIFIED: These may vary by plan -->
...
|---|---|---|---|
| `micro` | 0.25 | 0.25 GB | Lightweight tasks (API calls, notifications) |
| `small-1x` | 0.5 | 0.5 GB | General purpose (default) |
| `small-2x` | 1 | 1 GB | Medium compute (data processing) |
| `medium-1x` | 1 | 2 GB | Memory-intensive (large datasets) |
| `medium-2x` | 2 | 4 GB | Heavy compute (ML inference) |
| `large-1x` | 4 | 8 GB | Very heavy compute (video processing) |
| `large-2x` | 8 | 16 GB | Maximum compute (large models) |

### 8.6 Rate Limits

<!-- UNVERIFIED -->

The REST API and SDK enforce rate limits per secret key. Rate-limited responses return HTTP 429 with `Retry-After` headers. The SDK automatically retries rate-limited requests with exponential backoff.
...
| Topic | File |
|---|---|
| REST API Overview | [`../01-api-docs/00-overview/00-overview.md`](../01-api-docs/00-overview/00-overview.md) |
| API Authentication | [`../01-api-docs/00-overview/01-authentication.md`](../01-api-docs/00-overview/01-authentication.md) |
| Errors & Retries (API) | [`../01-api-docs/00-overview/02-errors-and-retries.md`](../01-api-docs/00-overview/02-errors-and-retries.md) |
| Auto-Pagination | [`../01-api-docs/00-overview/03-auto-pagination.md`](../01-api-docs/00-overview/03-auto-pagination.md) |
| Tasks API | [`../01-api-docs/01-tasks-api/`](../01-api-docs/01-tasks-api/) |
| Batches API | [`../01-api-docs/02-batches-api/`](../01-api-docs/02-batches-api/) |
| Runs API | [`../01-api-docs/03-runs-api/`](../01-api-docs/03-runs-api/) |
| Queues API | [`../01-api-docs/04-queues-api/`](../01-api-docs/04-queues-api/) |
| Schedules API | [`../01-api-docs/05-schedules-api/`](../01-api-docs/05-schedules-api/) |
| Env Vars API | [`../01-api-docs/06-env-vars-api/`](../01-api-docs/06-env-vars-api/) |
| Deployments API | [`../01-api-docs/07-deployments-api/`](../01-api-docs/07-deployments-api/) |
| Waitpoints API | [`../01-api-docs/08-waitpoints-api/`](../01-api-docs/08-waitpoints-api/) |
| Query API | [`../01-api-docs/09-query-api/`](../01-api-docs/09-query-api/) |
