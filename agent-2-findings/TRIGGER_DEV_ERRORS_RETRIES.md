...
> Canonical reference for error handling and retry behavior in Trigger.dev SDK 4.4.3.
...
| `{ retry: { ... } }` | Override the retry options for the next retry attempt. <!-- UNVERIFIED --> |
| `{ error: Error }` | Replace the error with a different one. <!-- UNVERIFIED --> |
...
<!-- UNVERIFIED: Whether config-level catchError can return the same full set of override objects as task-level catchError -->
...
<!-- UNVERIFIED: exact task-level onFailure signature -->
...
- [API Reference -- Errors & Retries](../01-api-docs/00-overview/02-errors-and-retries.md)
