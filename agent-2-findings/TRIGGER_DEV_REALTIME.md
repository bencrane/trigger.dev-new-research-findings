...
> Canonical reference for Trigger.dev Realtime -- subscribing to run updates, streaming data, and using React hooks. SDK 4.4.3.
...
<!-- UNVERIFIED: exact createPublicToken API shape may vary; confirm scopes object structure against latest SDK -->
...
<!-- UNVERIFIED: exact expirationTime format and default duration -->
...
<!-- UNVERIFIED: full list of boolean helpers; there may be additional ones like isQueued, isExecuting -->
...
| `options.enabled` | `boolean` | No | Whether the subscription is active. Defaults to `true`. <!-- UNVERIFIED --> |
...
<!-- UNVERIFIED: exact useWaitToken API; this hook may be named differently in some SDK versions -->
...
<!-- UNVERIFIED: whether both .pipe() and .write() are available; the docs show .pipe() for Streams v2 -->
...
<!-- UNVERIFIED: exact integration pattern with Vercel AI SDK streamText and streams.pipe -->
...
<!-- UNVERIFIED: exact useRealtimeStream API signature and return shape -->
...
<!-- UNVERIFIED: exact subscribeToRun API -- it may return an AsyncIterable or use a callback pattern -->
...
<!-- UNVERIFIED: exact subscribeToRunsWithTag API -->
...
<!-- UNVERIFIED: exact metadata.set API; may be metadata.update or similar -->
...
- [API Reference -- Retrieve Run](../01-api-docs/03-runs-api/01-retrieve-run.md)
- [API Reference -- Stream Batch Items](../01-api-docs/02-batches-api/03-stream-batch-items.md)
